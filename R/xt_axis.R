...
#' [xt_arrange_downstream()], [xt_distance_downstream()], [xt_gradient()], etc.
#' Channels built with [xt_generate_plan()] store the sampling axis
#' automatically on the [`xchan`] object.
#'
...
#'
#' **What “downstream” means:** [xt_distance_downstream()],
#' [xt_arrange_downstream()], and related helpers treat **increasing distance
#' along the stored `LINESTRING`**, measured from its **first vertex to its
#' last** (vertex digitization order), as downstream. The package does **not**
#' infer flow from a DEM, network topology, or a separate flag—only from this
#' polyline. Use [xt_reverse_flow()] to reverse the axis (and flip transects) so
#' the same geographic line is traversed from the opposite end.
#'
...
#' defined relative to the downstream **tangent** of the axis at the station
#' where the extended bank-to-bank chord meets the axis (same cross-product rule
#' as [xt_generate_plan()]). After the axis is set, each transect is
#' **re-oriented** if needed (end-for-end) so vertex order matches that
#' convention when the axis pierces the segment from the “wrong” side, matching
#' profiles are mirrored so profile distance still increases toward the right
#' bank, and sections are **re-sorted** with [xt_arrange_downstream()] by
#' increasing projected chainage. A highly curved, U-shaped, or self-overlapping
#' axis can still yield counter-intuitive chainages; the package does not
#' validate global monotonicity against terrain.
#'
...
#' Re-orient transects to the stored axis and sort by chainage (after
#' `xt_axis<-`)
#'
#' @noRd
