#' Splitter Schemes
#'
#' Splitters are functions that determine how much of the widening
#' applied to a channel is allocated to the left and right banks. They are
#' functions of cross sections, and return a numeric vector of proportions
#' indicating how much of the widening should be applied to the left bank
#' (left, by convention).
#'
...
#' @returns A splitter object. See `new_splitter()` for details.
#' @rdname splitters
#' @export
splitter_left <- function(prop = 1) {
...
#' @rdname splitters
#' @export
splitter_right <- function(prop = 1) {
  checkmate::assert_numeric(prop, lower = 0, upper = 1, any.missing = FALSE)
  f <- function(cross_sections) {
...
#' @rdname splitters
#' @export
splitter_both <- function(prop_left = 0.5, prop_right = 0.5) {
