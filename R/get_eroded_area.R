...
get_eroded_area <- function(xsec1, xsec2){
  ...
  xsec1_df <- data.frame(relative_distance = c(xsec1[["left"]][["multiline"]][,1],xsec1[["right"]][["multiline"]][,1] ),
  ...
  xsec2_df <- data.frame(relative_distance = c(xsec2[["left"]][["multiline"]][,1],xsec2[["right"]][["multiline"]][,1] ),
