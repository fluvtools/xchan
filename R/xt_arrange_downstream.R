...
#' ([xt_distance_downstream()]). After [xt_reverse_flow()], the stored axis (if
#' any) is reversed, so that zero chainage is at the opposite end of the same
#' geographic line — increasing distance is then hydrologic downstream with
#' reversed flow. `xt_arrange_upstream()` uses **decreasing** distance from the
#' axis start.
#'
#' @param channel An [`xchan`][xchan()] object.
#' @param axis Optional LINESTRING (`sfc` / `sfg`) for projection ordering (same
#'   rules as
#'   [xt_distance_downstream()]).
#'
#' @returns An [`xchan`] with sections reordered along the axis (attributes
#'   `crs` and
#'   `axis` preserved).
#'
#' @details
#' Ordering uses the same chainage as [xt_distance_downstream()]: intersection
#' of the extended bank-to-bank chord with the axis (nearest intersection to the
#' bank midpoint when there are several), otherwise the nearest point on the
#' axis to the bank midpoint.
#'
#' **Reverse flow:** [xt_reverse_flow()] does not permute sections but reverses
#' the stored axis, so `xt_arrange_downstream()` orders sections for the new
#' downstream direction. [elevation_bank()] with default `min` is unchanged
#' until you reorder.
#'
