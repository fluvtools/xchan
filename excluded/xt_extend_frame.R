...
#' channel <- xt_extend_frame(
#'   channel,
#'   extender = extender_flat(extent = 20, elevation = elevation_bank_left())
#' )
#' @export
xt_extend_frame <- function(channel, extender) {
  ...
  for (i in seq_along(profile)) {
    ...
    left_extension <- matrix(
      c(
        seq(
          left_bank_dist - extender_params$extent,
          left_bank_dist,
          length.out = 5
        ),
    ...
    right_extension <- matrix(
      c(
        seq(
          right_bank_dist,
          right_bank_dist + extender_params$extent,
          length.out = 5
        ),
  ...
  # Update the channel
  channel = set_channel_profile(channel, profile)
  channel
}
