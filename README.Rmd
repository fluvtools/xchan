...
**Data model.** A channel is a data frame (`xchan_tbl`) with one row per cross
section. Each row stores an `xsection` (planimetric geometry and optional
long-profile data). `channel_plan()` and `channel_profile()` return derived
views for plotting and analysis; CRS is held once on the channel, like an `sfc`
collection.
