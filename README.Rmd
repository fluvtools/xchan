...
The package includes spatial data for the Squamish River in British Columbia: a bankline polygon and a DEM. It's possible to create channels in other ways; see the [_Getting Started with Channels_]() and [_Channels with Profiles_]() vignettes for more information.
...
```{r}
grad <- xt_gradient(widened_squamish, reference = elevation_bank(min))
grad
```
...
To learn more about channel computations like these, see the [_Channel Computations_]() vignette.
