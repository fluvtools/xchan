test_that("Eroded volume is correct when eroding into a gully.", {
  ...
  area1 <- area0.5
  expect_equal(xt_erosion_volume_left(xs, 1), area1)
  area1.5 <- area1
  expect_equal(xt_erosion_volume_left(xs, 1.5), area1.5)
  area2 <- area1.5 + 0.5 * 1 / 2
  expect_equal(xt_erosion_volume_left(xs, 2), area2)
  area3 <- area2 + 1 * 1
  expect_equal(xt_erosion_volume_left(xs, 3), area3)
  area4 <- area3 + 0.5 * 1 + 0.5 * 1 / 2
  expect_equal(xt_erosion_volume_left(xs, 4), area4)
  ...
  expect_equal(xt_erosion_volume_left(xs, 1.5 - dw), area1.5 - gone_area)
  expect_equal(xt_erosion_volume_left(xs, 2 - dw), area2 - gone_area)
  expect_equal(xt_erosion_volume_left(xs, 3 - dw), area3 - gone_area)
  expect_equal(xt_erosion_volume_left(xs, 4 - dw), area4 - gone_area)
})
