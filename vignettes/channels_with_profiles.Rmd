...
recommend first reading the [_Getting Started with
Channels_](basic_channels.html) vignette.
...
An important feature of profile-enabled cross sections is that __they
extend outside of the banks__. Here is a planimetric view of the channel
...
below we follow `xt_generate_profile()` with `xt_dredge_to()` and a
rectangular bathymetry specification (`bathy_rectangle(depth = 3)`) to
insert a synthetic 3 m deep channel.
...
generate planimetric cross sections (as we did in the [_Getting Started
with Channels_](basic_channels.html) vignette).
...
Now generate the profile cross sections with `xt_generate_profile()`,
